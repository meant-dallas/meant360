...
import { uploadSponsorLogo, deleteFile } from '@/lib/blob-storage';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await uploadSponsorLogo(buffer, file.name, file.type);

    return jsonResponse(result, 201);
  } catch (error) {
    console.error('POST /api/upload/sponsor-logo error:', error);
    Sentry.captureException(error, { extra: { context: 'Sponsor logo upload POST' } });
